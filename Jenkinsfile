pipeline {
    ...
    environment {
        ...
        EC2_HOST = "65.0.103.122"      // Replace with your EC2 instance IP
        APP_DIR = "/home/ubuntu/ec2-jenkins/app" // Path to app directory on EC2
        SSH_KEY = "/var/lib/jenkins/appkey.pem"    // Path to your EC2 private key
    ...
    steps {
        script {
            ...
            sh '''
            scp -o StrictHostKeyChecking=no -i /var/lib/jenkins/appkey.pem -r app/main.py app/templates ubuntu@65.0.103.122:/home/ubuntu/ec2-jenkins/app/
            '''
            
            echo "Restarting application on EC2..."
            sh '''
            ssh -o StrictHostKeyChecking=no -i /var/lib/jenkins/appkey.pem ubuntu@65.0.103.122 <<EOF
