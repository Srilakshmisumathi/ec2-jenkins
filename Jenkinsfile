pipeline {
    ...
    environment {
        ...
        EC2_HOST = "13.203.158.222"      // Replace with your EC2 instance IP
    ...
    steps {
        script {
            ...
            sh '''
            scp -o StrictHostKeyChecking=no -i /var/lib/jenkins/sri.pem -r app/main.py app/templates ubuntu@13.203.158.222:/home/ubuntu/ec2-jenkins/app/
            '''
            
            echo "Restarting application on EC2..."
            sh '''
            ssh -o StrictHostKeyChecking=no -i /var/lib/jenkins/sri.pem ubuntu@13.203.158.222 <<EOF
